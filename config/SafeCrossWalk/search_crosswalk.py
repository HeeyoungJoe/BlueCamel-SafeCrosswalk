...
def getNearestCrossWalk(current_location, CRlocations, city = "Seoul"):
    ...
    distances = [haversine(current_location, CRlocations[id])*1000 for current_location, id in zip(current_location,vectorids) ]
    return distances
if __name__ == "__main__":
    ...
    testqueries = [37.5764942,126.9439352]
    testqueries = [37.559175, 126.940603]
    testqueries = [37.5550412,126.9366316]
 #    vectorids = getNearestCrossWalk(index, ivindex, testqueries, k = 1)
   # targetids = np.concatenate(vectorids)
    ...
    distance = getNearestCrossWalk(current_location=[testqueries], CRlocations=testset)
    print(distance)